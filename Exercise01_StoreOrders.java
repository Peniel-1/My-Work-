...
public class Exercise01_StoreOrders {
    ...
    public int getCalzoneSales(int[] orders) {
        ...
        int count = 0; // initialize number of calzone sold for the day to 0

        // loop over the input array
        for (int i = 0; i < orders.length; i++) {
            if (orders[i] == CALZONE) // ith order is for calzone, increment count by 1
                count++;
        }
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        ...
        int totalRevenue = 0; // initialize total revenue for cheese pizzas to 0

        // loop over the input array
        for (int i = 0; i < orders.length; i++) {
            if (orders[i] == SMALL_CHEESE) // ith order is for small cheese pizza
                totalRevenue += 8; // add 8 to totalRevenue
            else if (orders[i] == MEDIUM_CHEESE) // ith order is for medium cheese pizza
                totalRevenue += 11; // add 11 to totalRevenue
            else if (orders[i] == LARGE_CHEESE) // ith order is for large cheese pizza
                totalRevenue += 14; // add 14 to totalRevenue
        }
