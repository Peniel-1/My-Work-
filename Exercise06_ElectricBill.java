...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        ...
        return ((EXCESS_UNITS_LIMIT * BASE_RATE) + (excessUnits * EXCESS_RATE));
    }
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        return hasRenewableEnergy ? (calculateElectricBill(unitsUsed)*DISCOUNT_FACTOR) :
     calculateElectricBill(unitsUsed);
    }
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        double actualUnitsUsed = (unitsReturned > 0)? (unitsUsed - unitsReturned):unitsUsed;
        if(actualUnitsUsed < 0){
            return actualUnitsUsed * BASE_RATE;
        }
        if(actualUnitsUsed == unitsUsed) {
            return calculateElectricBill(unitsUsed);
        }
            return calculateElectricBill(actualUnitsUsed,true);
        }
    }
